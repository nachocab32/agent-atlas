        ...
        <Tabs defaultValue="resumen">
          ...
          <TabsContent value="resumen" className="flex flex-col gap-8">
            ...
            <section className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-3"><div className="flex items-center gap-2"><Play className="size-4 text-primary" /><h2 className="text-sm font-medium">Demo</h2></div><span className="text-xs text-muted-foreground">{etapa.video.duracion}</span></div>
              <div className="aspect-video bg-muted"><iframe className="size-full" src={`https://player.vimeo.com/video/${etapa.video.vimeoId}`} title={`Demo de ${etapa.nombre}`} allow="autoplay; fullscreen; picture-in-picture" allowFullScreen /></div>
            </section>
          ...
          <TabsContent value="requisitos" className="flex flex-col gap-8">
            <section className="flex flex-col gap-4"><Titulo>Cómo se invoca</Titulo><p className="leading-relaxed text-muted-foreground">{etapa.comoSeInvoca}</p><pre className="overflow-x-auto rounded-xl border border-border bg-neutral-950 p-4 font-mono text-xs leading-relaxed text-white">{etapa.promptEjemplo}</pre></section>
